...
function TreePainter(senTree, rootAnchor) {
	...
	this.paintInterval = 150;      // number of ms between paint steps
	...
	this.paint = function(node) {
        ...
        var formula;
        if (node.formula[0] == tc.NEGATION) {
            formula = "⊣&nbsp;" + translator.fla2html(node.formula[1]);
        } else {
            formula = "⊢&nbsp;" + translator.fla2html(node.formula);
        }
		var html = "<span class='" + this.formulaCSS + "'>" + formula + "</span>";
		html += "&nbsp;&nbsp;#" + curNodeNumber;
